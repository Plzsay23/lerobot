...
def async_client(cfg: RobotClientConfig):
    ...
    # 1. 환경 변수에서 구동할 ACT 모델 리스트 동적 로드
    # 사용 예: export ACT_MODELS="user/pick_place,user/cleaning"
    models_env = os.environ.get("ACT_MODELS", "dummy/model1,dummy/model2")
    available_models = [m.strip() for m in models_env.split(",") if m.strip()]
    num_classes = len(available_models) + 1 # 마지막 인덱스는 '작업 없음(None)'
    
    print("\n" + "="*60)
    print(f"🌍 [Config] 장착된 ACT 모델 리스트 ({len(available_models)}개):")
    for i, m in enumerate(available_models):
        print(f"   - Index {i}: {m}")
    print(f"   - Index {len(available_models)}: (작업 없음 / 무시)")
    print("="*60)

    # 2. 범용 VLM 에이전트 초기화
    vlm_agent = ManagerAgent(num_classes=num_classes)

    # 3. 로봇 하드웨어 초기화 (프로그램 켤 때 딱 한 번)
    print("\n[System] 로봇 하드웨어 및 카메라 초기화 중...")
    shared_robot = make_robot_from_config(cfg.robot)
    shared_robot.connect()
    
    # 카메라 키 찾기 (예: 'observation.images.top')
    camera_keys = [k for k in shared_robot.camera_features.keys()]
    main_camera_key = camera_keys[0] if camera_keys else None
    
    print("\n✅ 모든 시스템 준비 완료. Interactive Agent Ready!")

    # 4. 메인 루프
    while True:
        try:
            print("\n" + "-"*60)
            user_input = input("⌨️ 사용자 명령을 입력하세요 (종료 'q'): ").strip()
