...
class RobotClient:
    ...
    def control_loop(self, task: str, max_duration: int = 30, verbose: bool = False) -> tuple[Observation, Action]:
        ...
        # [추가] 자동 종료(Task 완료) 감지를 위한 변수
        self.stationary_time = 0.0
        self.prev_positions = None
        self.last_obs_time = start_time
        ...
        while self.running:
            ...
            if self._ready_to_send_observation():
                ...
                # [핵심 추가 로직] 로봇이 작업을 마치고 멈췄는지 자동 감지
                # 로봇이 막 움직이기 시작할 때 꺼지는 것을 막기 위해 '최소 5초' 이후부터 검사
                if elapsed_time > 5.0 and _captured_observation is not None:
                    # 현재 관절 위치 추출
                    current_positions = []
                    for key in self.robot.action_features:
                        val = _captured_observation.get(key, _captured_observation.get(f"observation.{key}", 0))
                        if hasattr(val, "item"): val = val.item()
                        current_positions.append(float(val))
                    
                    if self.prev_positions is not None:
                        # 직전 프레임의 위치와 비교하여 얼마나 움직였는지(Delta) 계산
                        max_diff = max(abs(c - p) for c, p in zip(current_positions, self.prev_positions))
                        
                        # 관절 변화량이 0.5도 미만이면 "제자리에 멈춰있다"고 판단
                        if max_diff < 0.5:
                            self.stationary_time += (current_time - self.last_obs_time)
                        else:
                            self.stationary_time = 0.0 # 조금이라도 움직이면 타이머 초기화
                            
                        # 2초 연속으로 가만히 있으면? -> 작업 끝 루프 강제 탈출
                        if self.stationary_time > 2.0:
                            print(f"\n[작업 완료 감지] 로봇이 {elapsed_time:.1f}초 만에 기본 자세로 복귀하여 정지했습니다.")
                            print("AI 제어 루프를 종료하고 다음 명령을 기다립니다...")
                            break
